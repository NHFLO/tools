...
def get_peilen(datadir, fname="PREP2XL_v122_Pandpeilen_kopie.xlsx"):
    ...
    peilen = pd.read_excel(
        os.path.join(datadir, "panden", fname),
        skiprows=list(range(7)) + [8, 9],
        engine="openpyxl",
        usecols=[0, 8, 10],
        index_col=0,
    )
    peilen.rename(
        columns={"Pandniveau pand 9": "pp_pand9", "Pandniveau middenkom": "pp_mkom"},
        inplace=True,
    )
...
def get_panden(model_ds, panden, peilen, modelgrid, cachedir=None, use_cache=False):
    ...
    """
    model_ds = nlmod.util.get_cache_netcdf(
        use_cache,
        cachedir,
        "panden_model_ds.nc",
...
def add_panden_to_model_ds(
    ...
    if model_ds.gridtype != "unstructured":
        warnings.warn("this function is not yet tested for structured grids")
    ...
    for i, row in panden.iterrows():
        area_pol = nlmod.mdims.polygon_to_area(
    ...
    model_ds_out = nlmod.mdims.get_model_ds_empty(model_ds)
    ...
    if (model_ds.gridtype == "structured") and model_ds.steady_state:
        empty_time_array = np.zeros((model_ds.dims["y"], model_ds.dims["x"])) * np.nan
        model_ds_out[f"{name}_peil"] = xr.DataArray(
        ...
        empty_time_array = (
            np.zeros((model_ds.dims["y"], model_ds.dims["x"], model_ds.dims["time"]))
            * np.nan
        )
        model_ds_out[f"{name}_peil"] = xr.DataArray(
            empty_time_array,
        ...
        model_ds_out[f"{name}_peil"] = xr.DataArray(
            empty_time_array, dims=("cid"), coords={"cid": model_ds.cid}
        )
    elif (model_ds.gridtype == "unstructured") and (not model_ds.steady_state):
        empty_time_array = (
            np.zeros((model_ds.dims["cid"], model_ds.dims["time"])) * np.nan
        )
        model_ds_out[f"{name}_peil"] = xr.DataArray(
            empty_time_array,
            dims=("cid", "time"),
            coords={"time": model_ds.time, "cid": model_ds.cid},
        )
    ...
    for col in peilen.columns:
        ...
        if pandpeil_ts.index[-1] < end:
            logger.warning(
                f"no pandpeil available after {pandpeil_ts.index[-1]} using average pandpeil NAP+{pandpeil_average:.2f} after this date"
            )
            pandpeil_ts.loc[end] = pandpeil_average
            pandpeil_ts.loc[
                pandpeil_ts.index[-1] + dt.timedelta(days=1)
            ] = pandpeil_average
            pandpeil_ts.sort_index(inplace=True)
        elif pandpeil_ts.index[0] > start:
            logger.warning(
                f"no pandpeil available before {pandpeil_ts.index[0]} using average pandpeil NAP+{pandpeil_average:.2f} before this date"
            )
            pandpeil_ts.loc[start] = pandpeil_average
            pandpeil_ts.loc[
                pandpeil_ts.index[0] - dt.timedelta(days=1)
            ] = pandpeil_average
            pandpeil_ts.sort_index(inplace=True)

        if model_ds.steady_state:
            pandpeil_ts = pd.Series(index=[0], data=[pandpeil_average])
            model_ds_out[f"{name}_peil"] = xr.where(
                peil == col, pandpeil_average, model_ds_out[f"{name}_peil"]
            )
        else:
...
def model_dataset_to_panden_riv(model_ds, gwf, name="pand_riv"):
    ...
    if model_ds.steady_state:
        riv_spd = nlmod.mdims.data_array_unstructured_to_rec_list(
            model_ds,
