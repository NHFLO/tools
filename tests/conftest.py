...
def _hygiene(monkeypatch):
    """Keep tests independent: no leaked figures, open files or data-location env var.

    ``NHFLODATA_LOCATION`` is deleted so a developer's local data mount can never
    redirect the tests away from the mockup data that CI resolves.
    ...
    """
    monkeypatch.delenv("NHFLODATA_LOCATION", raising=False)
    yield
    plt.close("all")
