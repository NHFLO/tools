...
The polder DRN also gains an optional ``exclude`` mask so carved lake cells can be handed
over to a dedicated stage boundary. The reach must be dropped whether its stage comes from
real HHNK peilgebied data or from the maaiveld fallback, which is why the exclusion nulls
the conductance after the celldata assignment rather than only editing the fallback mask.
"""
...
# Cell ids of the 3x3 disv_grid used by the exclude tests.
